...
private suspend fun postRules(rules: KrShortRules, editKey: String): KrShortRules = withContext(Dispatchers.IO) {
    val body = httpJson("POST", RULES_URL, rules.toJson().toString(), editKey)
    parseRules(JSONObject(body).optJSONObject("rules") ?: JSONObject())
}

private suspend fun runScan(editKey: String): RunScanResult = withContext(Dispatchers.IO) {
    val body = httpJson("POST", RUN_SCAN_URL, "{}", editKey)
