...
def _get_dynamic_kr_stock_universe() -> pd.DataFrame:
    ...
    return _select_universe_candidates(market_df, int(settings.kr_universe_top_n))
...
def _infer_sector(code: str, name: str) -> str:
    ...
    if code in known:
        return known[code]
    sector_rules = [
        ('반도체장비/소재', ['반도체', '하이닉스', 'HPSP', 'ISC', '리노공업', '테크윙', '원익', '솔브레인', 'DB하이텍', '주성엔지니어링', '피에스케이', '이오테크닉스', '한미반도체', '동진쎄미켐']),
