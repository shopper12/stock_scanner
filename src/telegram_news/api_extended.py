...
def post_bot_command(req: BotCommandRequest, x_api_key: str | None = Header(default=None)) -> dict:
    ...
    msg = handle_command(user_id=req.user_id or "default", message=req.message or "", latest_report=latest)
    return {"ok": True, "message": msg, "kind": data.get("kind"), "hours": data.get("hours"), "source": data.get("source"), "generated_at": data.get("generated_at")}
